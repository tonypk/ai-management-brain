...
export async function updateBlend(primaryId: string, secondaryId: string, weight: number) {
  return request<{ data: any }>('/mentor/blend', {
    method: 'PUT',
    body: JSON.stringify({ primary_id: primaryId, secondary_id: secondaryId, weight }),
  })
}
